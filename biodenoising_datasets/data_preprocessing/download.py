...
def download_from_remote(remote, save_dir, force_overwrite):
    ...
    if not os.path.exists(download_path) or force_overwrite:
        ...
        if os.path.exists(download_path) and len(remote.checksum)>0:
            os.remove(download_path)
        
        if remote.url.startswith("s3"):
            command = "aws --no-sign-request s3"
            if len(remote.checksum)>0:
                command += " cp " 
                command += remote.url + " " + download_dir
            else:
                command += " sync "
                command += remote.url + " " + download_path
            try:
                os.system(command)
            except Exception as e:
                logging.error("Failed to download dataset from {}!".format(remote.url))
                raise e

            if len(remote.checksum)>0:
                full_path_to_filename = os.path.join(download_dir, remote.filename)
                if not os.path.exists(full_path_to_filename):
                    raise RuntimeError("{} does not exist".format(full_path_to_filename))
        elif remote.url.startswith("gs"):
            ...
            with DownloadProgressBar(
                ...
                try:
                    ...
                    error_msg = """
                                failed to download the dataset from {}!
