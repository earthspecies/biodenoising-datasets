'''
It is not clear what method was used to clean these orca sounds and they are not 100% clean so they should not be used in training.
'''
import os
...
class Dataset(AudioDataset):
    def __init__(self, conf, path, dname='orcasound_sep', *args, **kwargs):
        self.path = path
        ...
        for k,path in self.all_dirs.items():
            self.audio_files[k] = [os.path.join(root,file) for root,fdir,files in os.walk(path) for file in files if file.endswith(tuple(ALLOWED_EXTENSIONS)) and 'orca' in file and not file.startswith('.')]
